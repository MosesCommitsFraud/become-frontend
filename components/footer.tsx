...
export default function CombinedFooter() {
    ...
    // Get current pathname to check if we're on the packages page
    const pathname = usePathname();
    const hideFloatingFooter = pathname === '/packages';

    useEffect(() => {
        // Don't run footer positioning logic if floating footer is hidden
        if (hideFloatingFooter) return;
    ...
    return (
        <>
            {/* Floating Footer - Only render if not on packages page */}
            {!hideFloatingFooter && (
                <div
                    ref={fixedFooterRef}
